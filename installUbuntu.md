...
## 言語の選択
使用する言語を選択します。デフォルトでは[日本語]に設定されています。
...
[今すぐ再起動する]をクリックして再起動を行います。

:::caution
Ubuntuのロゴとロードマークが表示されながら「Please remove the installation medium, then press ENTER:(インストールメディアを取り出し、ENTER キーを押してください。)」と表示されます。  
Enterを押してマシンが完全に停止してください。
:::
...
## ログイン
再起動をするとログイン画面が現れます。rootでログインを行うため、[アカウントが見つかりませんか？]を選択しrootでログインします。
...
15行目付近にある以下のコメントを外し、末尾に `use_uid` を付けます。
