...
## 言語選択
インストールメディアが起動すると言語選択メニューが表示されます。左のメニューの中央付近にある**[日本語]**を選択し、**[続行]**を選択します。
...
さらに、**[設定]**を選択し、**[IPv4の設定]**をクリックし、メソッドを**自動(DHCP)**から**手動**に変更します。
...
|ネットマスク|24|
|ゲートウェイ|{{gatewayIP}}|
|追加のDNSサーバー|{{gatewayIP}}|
|追加のドメイン検索|netsys.cs.t-kougei.ac.jp, cs.t-kougei.ac.jp, t-kougei.ac.jp|
...
追加した`ntp-a.t-kougei.ac.jp`の**[稼働中]**の項目欄にチェックがあれば追加は完了です。
`ntp-a.t-kougei.ac.jp`の**[使用]**と**[プール]**の項目欄チェックを入れ、それ以外の使用中のチェックを外します。
...
## ログイン
ログイン画面が表示されたら、ログインを行います。rootでログインを行うため、**[アカウントが見つかりませんか？]**を選択しrootでログインします。
...
### yumのプロキシ設定
{question:RedHat系Linuxで使用される高機能なパッケージマネージャの名前はなんですか？}{answer:yum}{hint:Yellowdog Updater Modifiedの略称}
パッケージマネージャーyumの設定を行います。

```bash
root@{{serverHostname}}:~# vi /etc/yum/yum.conf
```
...
```bash
root@{{serverHostname}}:~# yum -y update 
```
...
### クライアントの起動とログイン
しばらくするとクライアントが自動的に起動します。

起動したクライアントにログインを行い構築したサービスのメールテストを行います。
...
```bash
root@{{serverHostname}}:~# yum -y install dovecot
```
:::
